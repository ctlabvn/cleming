...
export default class DealManager extends Component {
    ...
    _formatIncreaseDecrease = (number) => {
      ...
      if (number >=0) return  <Text success style={{fontSize: 12}}>&#8593; {Math.abs(number).toFixed(2)}%</Text>
      else return <Text warning style={{fontSize: 12}}>&#8595; {Math.abs(number).toFixed(2)}%</Text>
    }
